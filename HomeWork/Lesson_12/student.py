...
class CheckName:
    ...
    def __set__(self, instance, value):
        if not value.isalpha() or not value.istitle():
            raise ValueError(f'Ошибка в имени студента {value = }! Cтрока должна содержать только буквы '
...
class Student:
    """
    Класс реализует модель студент-успеваемость. При создании объекта, создается журнал успеваемости для него с
    получением средне-арифметических значений
    """
    surname = CheckName()
    name = CheckName()
    ...
    def is_subject_in_grade_book(self, subject: str):
        ...
        if subject not in self.__grade_book.keys():
            raise ValueError(f'Ошибка! {subject = } Такого предмета нет в журнале оценок')
        return True
    ...
    def add_grade(self, subject: str, grade: int):
        ...
        if grade not in range(2, 6):
            raise ValueError(f'Ошибка! {grade = }, оценка должны быть в диапазоне от 2 до 5 баллов включительно')
    ...
    def add_test_result(self, subject: str, num_score: int):
        ...
        """
        if num_score not in range(0, 101):
            raise ValueError(f'Ошибка! {num_score = }, баллы должны быть в диапазоне от 0 до 100 включительно')
        if self.is_subject_in_grade_book(subject):
...
if __name__ == '__main__':
    ...
    student_1.add_test_result('алгебра', 55)
